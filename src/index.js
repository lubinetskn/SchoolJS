...
function findAllPSiblings(where) {
    return [...where.querySelectorAll('p')].map((item) => {
...
function collectDOMStat(root) {
    ...
    (function getStatistics(node) {
        [...node.childNodes].forEach((item) => {
            if (item.nodeType === Node.TEXT_NODE) {
                obj.texts++;
            } else {
                item.classList.forEach((itemClass) => {
                    typeof obj.classes[itemClass] === 'undefined' ?
                        obj.classes[itemClass] = 1 :
                        obj.classes[itemClass]++
                });
                typeof obj.tags[item.tagName] === 'undefined' ?
    ...
    new MutationObserver((mutations) => {
        mutations.forEach((mutation) => {
            fn({
                ...
                nodes: mutation.addedNodes.length > 0 ?
                    [...mutation.addedNodes] :
